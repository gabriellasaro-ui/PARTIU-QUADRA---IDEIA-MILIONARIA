...
8. **Gerente:** overview, agenda, aprovação, CRUD quadras, financeiro/repasse, mensalistas, cupons, avaliações → **concluída na F8**: `models/settlement.py` + `models/coupon.py`, migração `i4c5d6e7f8a9` (payments/booking `user_id` nullable + `client_*`), `repositories/gerente.py` + `services/gerente.py` + `schemas/gerente.py`, `api/gerente.py` com 23 rotas reais (dashboard, agenda, reservas + manual, mensalistas, financeiro, quadras, avaliações+resposta, cupons, perfil, configurações, desativação — tudo `get_current_manager`), task `gerar_settlements_semanais` (beat segunda 03:00), seed idempotente. Harness F8 (77 asserts) + regressão F4–F7 (24 asserts) verdes; Postgres externo migrado/seedado.
9. **Clubes/peladas/partidas:** CRUD, presença, partida ativa, placar/eventos → **concluída na F9**: `models/club.py` (clubs/club_members/club_messages), `models/pelada.py` (peladas UNIQUE `(source_booking_id, date_iso)` + pelada_attendance), `models/match.py` (matches + match_teams/players/events/media), migração `j5d6e7f8a9b1` (10 tabelas), `repositories`/`services`/`schemas`/`api` de clubes/peladas/partidas (+17 rotas, app → 90 /api), código de convite 6 chars sem hífen, um clube ativo por usuário, pelada nasce da reserva (mensalista gera 4 com +7 dias), partida materializada sob demanda na `ativa` com fase por timestamps, permissões organizador/participante, notificações `clube.entrou`/`pelada.criada`/`pelada.presenca`/`partida.presenca`/`partida.atraso`/`partida.encerrada`. Harness F9 (99 asserts) + regressão F4–F8 verdes; Postgres externo migrado/seedado (1 clube/5 membros, 5 peladas, 1 partida).
