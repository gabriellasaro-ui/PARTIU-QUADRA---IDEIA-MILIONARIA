...
# Comissao cobrada do dono da quadra no repasse, nao do jogador no checkout
TAXA_PLATAFORMA = 0.05

IMG = "https://images.unsplash.com/"
IMG_PARAMS = "?auto=format&fit=crop&w=900&q=80"

# "Usuarios logados" do mockup
...
# Coordenadas (mock) por quadra, em torno de Goiania, para a view de mapa.
COORDS = {
    1: (-16.7060, -49.2350), 2: (-16.7050, -49.2770), 3: (-16.6870, -49.2620),
    4: (-16.6780, -49.2720), 5: (-16.7150, -49.2470), 6: (-16.6950, -49.2650),
}
USER_LOC = (-16.6950, -49.2550)  # "voce esta aqui" (mock)
for _q in QUADRAS:
...
FATURAMENTO_SEMANA = [540, 620, 380, 740, 920, 1180, 860]  # Seg..Dom
...
# ---------------------------------------------------------------------------
# SEMENTES do estado mutável (persistido em store.py)
# ---------------------------------------------------------------------------
# Reservas do gerente. Status: Solicitada | Confirmado | Pago | Pendente | Recusada | Cancelada
SEED_RESERVAS = [
...
def clamp_dur(valor):
    """Duracao da reserva em horas, limitada a 1..3."""
    try:
...
def galeria_quadra(q):
    """Galeria de 3 fotos: a principal + 2 do pool."""
...
def reservas_jogador():
    """Reservas do jogador (mock), montadas a partir de algumas quadras."""
    plano = [
...
def line_chart(values, labels, w=600, h=212, pad_top=20, pad_bottom=30, pad_left=42, pad_right=20):
    """Geometria de um grafico de area/linha em SVG (sem JS)."""
    n = len(values)
