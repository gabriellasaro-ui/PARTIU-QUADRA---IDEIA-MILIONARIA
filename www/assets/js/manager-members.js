/* Mensalistas da arena — secao restaurada do app antigo.

   Ela existia, o menu tinha o item e o manager-app.js manteve os handlers,
   mas o markup se perdeu na refatoracao. Voltou ligada a dados.
...
const manuais = () => storage.get(KEY, []);

function linha({ id, name, court, day, time, price, origem }) {
  ...
  return `<article class="manager-member" data-member-id="${escapeHtml(id)}">
    ...
    <span class="status ${doApp ? 'pago' : 'confirmado'}">${doApp ? 'Pelo app' : 'Cadastro manual'}</span>
...
export async function renderManagerMembers(root) {
  ...
  const contador = root.querySelector('[data-member-count]');
  if (contador) contador.textContent = todos.length;

  const select = root.querySelector('[data-member-courts]');
  if (select && !select.options.length) {
    const quadras = await venueService.list({});
    select.innerHTML = quadras.map((q) => `<option>${escapeHtml(q.name)}</option>`).join('');
  }
